...
namespace VideoTeca.WebApi.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Login(UserLoginDto userLogin)
        {
            try
            {
                var user = await _userManager.FindByNameAsync(userLogin.nomeCompleto);
                ...
                if (result.Succeeded)
                {
                    var appUser = await _userManager.Users
                        .FirstOrDefaultAsync(u => u.NormalizedUserName == userLogin.nomeCompleto.ToUpper());

                    var userToReturn = _mapper.Map<UserLoginDto>(appUser);

                    return Ok(new
                    {
                        token = GerarJWToken(appUser).Result,
                        user = userToReturn
                    });
                }
        ...
        private async Task<string> GerarJWToken(User user)
        {
